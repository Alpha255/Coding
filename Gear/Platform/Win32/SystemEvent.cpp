...
void SystemEventHandler::processEvent()
{
	if (!m_EventData)
	{
		return;
...
bool8_t SystemEventHandler::dispatchEvent()
{
	::MSG msg = {};
...
bool8_t SystemEventHandler::isFocusWindow(uint64_t windowHandle)
{
	return ::GetCapture() == reinterpret_cast<::HWND>(windowHandle);
}

void SystemEventHandler::focusWindow(uint64_t windowHandle)
{
	::SetCapture(reinterpret_cast<::HWND>(windowHandle));
}

void SystemEventHandler::unFocusWindow()
{
	::ReleaseCapture();
}
