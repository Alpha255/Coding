...
::LRESULT messageProc(::HWND hWnd, uint32_t msg, ::WPARAM wParam, ::LPARAM lParam)
{
	...
	Application *app = reinterpret_cast<Application *>(::GetWindowLongPtrA(hWnd, 0));
	if (app)
	{
		struct win32Event
		{
		...
		SystemEventHandler::instance().pushEvent((byte *)&event, sizeof(win32Event));
...
void Application::makeWindow(const std::string &title, uint32_t width, uint32_t height, uint32_t extraWindowStyle, uint16_t iconID)
{
	...
	m_WindowSize = Math::Vec2((float32_t)width, (float32_t)height);
