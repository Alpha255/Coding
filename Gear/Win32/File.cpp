...
bool8_t File::isExists(const char8_t* path, bool8_t isDirectory)
{
#if 1
...
void makeFileList(
	_Inout_ std::vector<std::string>& result,
...
void tryToFindFile(const std::string& path, const std::string& name, _Inout_ std::string& result)
{
	std::string root(path + "\\*.*");
...
std::vector<std::string> File::getFileList(const char8_t* path, const std::vector<std::string>& filters, bool8_t lowercase)
{
...
void File::createDirectory(const char8_t* path)
{
	std::string temp(path);
	VERIFY_SYSTEM(ERROR_SUCCESS == ::SHCreateDirectory(nullptr, std::wstring(temp.cbegin(), temp.cend()).c_str()));
}
