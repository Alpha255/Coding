...
{
public:
	static TextureDesc load(const std::string& extension, const byte8_t* data, size_t size, bool8_t sRGB)
	{
		assert(data && size);

		auto format = getFormat(extension);
		assert(format != ETextureFormat::Unsupported);

		TextureDesc desc;
		if (format == ETextureFormat::DDS)
		{
			desc = std::move(load_DDS(data, size));
		}
		else if (format == ETextureFormat::KTX)
		{
			desc = std::move(load_KTX(data, size));
		}
		else
		{
			desc = std::move(load_General(data, size));
		}
...
protected:
	static ETextureFormat getFormat(const std::string& extension)
	{
...
#undef IS_BITMASK
	...
	static TextureDesc load_DDS(const byte8_t* data, size_t size)
	{
		...
		desc.Layers = header->depth == 0u ? 1u : header->depth;
		...
		if (dxt10Header)
		{
			...
			case D3D11_RESOURCE_DIMENSION_TEXTURE1D:
				...
				desc.Height = desc.Layers = 1u;
				desc.Type = desc.ArraySize > 1u ? ETextureType::T_1D_Array : ETextureType::T_1D;
				break;
			case D3D11_RESOURCE_DIMENSION_TEXTURE2D:
				if (dxtHeader->miscFlag & D3D11_RESOURCE_MISC_TEXTURECUBE)
				{
					desc.Type = desc.ArraySize > 1u ? ETextureType::T_Cube_Array : ETextureType::T_Cube;
					desc.ArraySize *= 6u;
					cubemap = true;
				}
				else
				{
					desc.Type = desc.ArraySize > 1u ? ETextureType::T_2D_Array : ETextureType::T_2D;
				}
				desc.Layers = 1u;
				break;
			case D3D11_RESOURCE_DIMENSION_TEXTURE3D:
				assert(header->flags & DDS_HEADER_FLAGS_VOLUME);
				desc.Type = ETextureType::T_3D;
				break;
			case D3D11_RESOURCE_DIMENSION_BUFFER:
				desc.Type = ETextureType::Buffer;
				break;
			}
		}
		else
		...
		case ETextureType::T_3D:
			...
			desc.Layers <= D3D11_REQ_TEXTURE3D_U_V_OR_W_DIMENSION);
			break;
		}

		/// gen mipmaps
		desc.Data = const_cast<byte8_t*>(bitData);
		desc.DataSize = bitSize;
		desc.Subresources.resize(static_cast<size_t>(desc.MipLevels) * static_cast<size_t>(desc.ArraySize));
		uint32_t index = 0u;
		for (uint32_t arrayIndex = 0u; arrayIndex < desc.ArraySize; ++arrayIndex)
		{
			uint32_t width = desc.Width;
			uint32_t height = desc.Height;
			uint32_t depth = desc.Layers;
	...
	static TextureDesc load_KTX(const byte8_t* data, size_t size)
	{
		...
		desc.DataSize = bytes;
		desc.Data = ktxTexture_GetData(ktx);
