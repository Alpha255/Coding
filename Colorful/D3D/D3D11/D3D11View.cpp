...
ID3D11Resource* D3D11TextureView::createTexture(ID3D11Device* device, const TextureDesc& desc)
{
	...
	{
		auto texture = create_shared<D3D11Texture1D>(device, desc);
	...
	{
		auto texture = create_shared<D3D11Texture2D>(device, desc);
		resource = texture->get();
		m_Texture = texture;
	}
	else if (desc.Type == ETextureType::T_3D)
	{
		auto texture = create_shared<D3D11Texture3D>(device, desc);
		resource = texture->get();
		m_Texture = texture;
	}
	...
	: D3D11TextureView(desc)
{
	...
	case ETextureType::T_2D:
		shaderResourceViewDesc.ViewDimension = desc.SampleCount > ESampleCount::SampleCount_1_Bit ? 
			D3D11_SRV_DIMENSION_TEXTURE2DMS : D3D11_SRV_DIMENSION_TEXTURE2D;
		if (desc.SampleCount > ESampleCount::SampleCount_1_Bit)
		{
	...
	: D3D11TextureView(desc, EDescriptorType::InputAttachment)
{
	...
	case ETextureType::T_1D_Array:
		...
		break;
	case ETextureType::T_2D:
		renderTargetViewDesc.ViewDimension = desc.SampleCount > ESampleCount::SampleCount_1_Bit ? 
	...
	: D3D11TextureView(desc, EDescriptorType::InputAttachment)
{
	...
	case ETextureType::T_1D_Array:
		...
		break;
	case ETextureType::T_2D:
		depthStencilViewDesc.ViewDimension = desc.SampleCount > ESampleCount::SampleCount_1_Bit ? 
