...
D3D11RasterizerState::D3D11RasterizerState(ID3D11Device* device, const RasterizationStateDesc& rasterizationStateDesc, const MultisampleStateDesc& multisampleStateDesc)
{
	...
	D3D11_RASTERIZER_DESC rasterizerDesc
	{
		...
		rasterizationStateDesc.EnableDepthBias ? static_cast<int32_t>(rasterizationStateDesc.DepthBias) : 0,
		rasterizationStateDesc.DepthBiasClamp,
		rasterizationStateDesc.DepthBiasSlope,
		rasterizationStateDesc.EnableDepthClamp,
		true, /// ScissorEnable
		multisampleStateDesc.SampleCount > ESampleCount::SampleCount_1_Bit,
		false
	};
...
D3D11BlendState::D3D11BlendState(ID3D11Device* device, const BlendStateDesc& blendStateDesc, const MultisampleStateDesc& multisampleStateDesc)
{
	...
	for (uint32_t i = 0u; i < ELimitations::MaxRenderTargets; ++i)
	{
		blendDesc.RenderTarget[i] = D3D11_RENDER_TARGET_BLEND_DESC
		{
			...
			D3D11EnumTranslator::blendFactor(blendStateDesc.ColorBlends[i].SrcColor),
			D3D11EnumTranslator::blendFactor(blendStateDesc.ColorBlends[i].DstColor),
			D3D11EnumTranslator::blendOp(blendStateDesc.ColorBlends[i].ColorOp),
			D3D11EnumTranslator::blendFactor(blendStateDesc.ColorBlends[i].SrcAlpha),
			D3D11EnumTranslator::blendFactor(blendStateDesc.ColorBlends[i].DstColor),
			D3D11EnumTranslator::blendOp(blendStateDesc.ColorBlends[i].AlphaOp),
			D3D11EnumTranslator::renderTargetWriteMask(blendStateDesc.ColorBlends[i].ColorMask)
		};
	}
...
D3D11DepthStencilState::D3D11DepthStencilState(ID3D11Device* device, const DepthStencilStateDesc& desc)
{
	...
	D3D11_DEPTH_STENCIL_DESC depthStencilDesc
	{
		...
		{
			D3D11EnumTranslator::stencilOp(desc.FrontFaceStencilState.FailOp),
			D3D11EnumTranslator::stencilOp(desc.FrontFaceStencilState.DepthFailOp),
			D3D11EnumTranslator::stencilOp(desc.FrontFaceStencilState.PassOp),
			D3D11EnumTranslator::compareFunc(desc.FrontFaceStencilState.CompareFunc),
		},
		{
			D3D11EnumTranslator::stencilOp(desc.BackFaceStencilState.FailOp),
			D3D11EnumTranslator::stencilOp(desc.BackFaceStencilState.DepthFailOp),
			D3D11EnumTranslator::stencilOp(desc.BackFaceStencilState.PassOp),
			D3D11EnumTranslator::compareFunc(desc.BackFaceStencilState.CompareFunc),
		},
	};
...
D3D11GraphicsPipeline::D3D11GraphicsPipeline(ID3D11Device* device, GraphicsPipelineState& state)
{
	assert(device);

	m_InputLayout = std::static_pointer_cast<D3D11InputLayout>(state.material()->inputLayout());
	m_RasterizerState = create_shared<D3D11RasterizerState>(device, state.rasterizationStateDesc(), state.multisampleStateDesc());
	m_BlendState = create_shared<D3D11BlendState>(device, state.blendStateDesc(), state.multisampleStateDesc());
	m_DepthStencilState = create_shared<D3D11DepthStencilState>(device, state.depthStencilStateDesc());
}
