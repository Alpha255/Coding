	...
	d3d12Device &device)
{
	...
	{
		uint32_t count = 0u;
		rVerifyD3D12(dxgiOutputs[i]->GetDisplayModeList1(format, DXGI_ENUM_MODES_INTERLACED, &count, nullptr));

		std::vector<DXGI_MODE_DESC1> tempDxgiModeDescs(count);
		rVerifyD3D12(dxgiOutputs[i]->GetDisplayModeList1(format, DXGI_ENUM_MODES_INTERLACED, &count, tempDxgiModeDescs.data()));
...
{
	rVerifyD3D12(m_Object->ResizeBuffers(
