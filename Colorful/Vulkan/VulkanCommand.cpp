...
VulkanCommandPool::VulkanCommandPool(VkDevice device, uint32_t queueIndex, VkCommandPoolCreateFlags flags)
	: m_Device(device)
{
	/*************************
	...
	**************************/
	...
	VERIFY_VK(vkCreateCommandPool(m_Device, &createInfo, VK_MEMORY_ALLOCATOR, reference()));
}
...
void VulkanCommandPool::reset()
{
	...
	VERIFY_VK(vkResetCommandPool(m_Device, get(), VK_COMMAND_POOL_RESET_RELEASE_RESOURCES_BIT));
}
...
{
	assert(isValid());
	assert(cmdBuffer->state() != VulkanCommandBuffer::EState::Pending);
