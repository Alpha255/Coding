...
VulkanFence::VulkanFence(VkDevice device, bool8_t signaled)
{
	...
	VERIFY_VK(vkCreateFence(device, &createInfo, VK_MEMORY_ALLOCATOR, reference()));
}
...
VulkanSemaphore::VulkanSemaphore(VkDevice device)
{
	...
	VERIFY_VK(vkCreateSemaphore(device, &createInfo, VK_MEMORY_ALLOCATOR, reference()));
...
VulkanEvent::VulkanEvent(VkDevice device)
{
	...
	VERIFY_VK(vkCreateEvent(device, &createInfo, VK_MEMORY_ALLOCATOR, reference()));
