...
float3 FxaaPixelShader(float2 pos, FxaaTex tex, float2 rcpFrame)
{
    ...
    float2 posP = posN;
    float2 offNP = horzSpan ? FxaaFloat2(rcpFrame.x, 0.0) : FxaaFloat2(0.0f, rcpFrame.y);
    ...
    if(directionN) 
	{
	    return FxaaFilterReturn(FxaaFloat3(1.0, 0.0, 0.0));
	}
    else           
	{
		return FxaaFilterReturn(FxaaFloat3(0.0, 0.0, 1.0));
	...
	/// Compute sub-pixel offset and fileter span 
	...
	if(ox < 0.0) 
	{
		return FxaaFilterReturn(FxaaLerp3(FxaaToFloat3(lumaO), FxaaFloat3(1.0, 0.0, 0.0), -ox));
	}
    
	if(ox > 0.0) 
	{
		return FxaaFilterReturn(FxaaLerp3(FxaaToFloat3(lumaO), FxaaFloat3(0.0, 0.0, 1.0),  ox));
	}
        
	if(oy < 0.0) 
	{
		return FxaaFilterReturn(FxaaLerp3(FxaaToFloat3(lumaO), FxaaFloat3(1.0, 0.6, 0.2), -oy));
	}
        
	if(oy > 0.0) 
	{
		return FxaaFilterReturn(FxaaLerp3(FxaaToFloat3(lumaO), FxaaFloat3(0.2, 0.6, 1.0),  oy));
	}
    
	return FxaaFilterReturn(FxaaFloat3(lumaO, lumaO, lumaO));
#endif
    float3 rgbF = FxaaTexLod0(tex, FxaaFloat2(pos.x + (horzSpan ? 0.0 : subPixelOffset), pos.y + (horzSpan ? subPixelOffset : 0.0))).xyz;
...
struct VSOutput_Fxaa
{
    float4 Pos : SV_POSITION;
    float2 Tex : TEXCOORD0;
};

VSOutput_Fxaa VS_Fxaa(uint id : SV_VertexID)
{
    VSOutput_Fxaa Output;
    Output.Tex = float2((id << 1) & 2, id & 2);
    Output.Pos = float4(Output.Tex * float2(2.0f, -2.0f) + float2(-1.0f, 1.0f), 0.0f, 1.0f);
    return Output;
}

float4 PS_Fxaa(VSOutput_Fxaa Input) : SV_TARGET
{
...
struct VSOutput_ShadowMap
{
    ...
    float4 PositionLightSpace : POSITION; 
    float3 Diffuse : COLOR0; 
    float2 TextureUV : TEXCOORD0; 
};

float4 VS_ShadowMap(VSInput_ShadowMap input) : SV_POSITION
{
...
//--------------------------------------------------------------------------------------
VSOutput_ShadowMap VS_Main(VSInput_ShadowMap input)
{
    VSOutput_ShadowMap Output;
...
float4 PS_Main(VSOutput_ShadowMap In) : SV_TARGET
{
