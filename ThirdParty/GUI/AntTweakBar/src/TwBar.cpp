        ...
        if( _DrawPart&DRAW_CONTENT )
        {
            ...
            for( h=0; h<nh; ++h )
            {
                ...
                else if( static_cast<CTwVarAtom *>(m_HierTags[h].m_Var)->m_Type==TW_TYPE_BUTTON && !m_IsPopupList )
                {
                    ...
                    {
                        int level = max(m_Font->m_CharHeight-6, 4); // space used by DrawHierHandles
                        Gr->DrawLine(m_PosX+m_VarX0+m_HierTags[h].m_Level*level, yh+m_Font->m_CharHeight/2, m_PosX+m_VarX2, yh+m_Font->m_CharHeight/2, m_ColSeparator );
                    }
                }
        ...
        else
        {
            ...
            else if( BarActive && m_HighlightedLine>=0 && m_HighlightedLine<(int)m_HierTags.size() && m_HierTags[m_HighlightedLine].m_Var )
            {
                if( _Key==TW_KEY_RIGHT )
                {
                    if( !m_HierTags[m_HighlightedLine].m_Var->IsGroup() ) 
                    {
                        CTwVarAtom *Atom_KeyRight = static_cast<CTwVarAtom *>(m_HierTags[m_HighlightedLine].m_Var);
                        bool Accept = !Atom_KeyRight->m_NoSlider || Atom_KeyRight->m_Type==TW_TYPE_BUTTON
                                      || Atom_KeyRight->m_Type==TW_TYPE_BOOL8 || Atom_KeyRight->m_Type==TW_TYPE_BOOL16 || Atom_KeyRight->m_Type==TW_TYPE_BOOL32 || Atom_KeyRight->m_Type==TW_TYPE_BOOLCPP
                                      || IsEnumType(Atom_KeyRight->m_Type);
                        if( !Atom_KeyRight->IsReadOnly() && !m_IsPopupList && Accept )
                        {
							Atom_KeyRight->Increment(+1);
                            if( g_TwMgr==NULL ) // Mgr might have been destroyed by the client inside a callback call
                                return 1;
                            m_HighlightClickBtnAuto = g_TwMgr->m_Timer.GetTime();
                            NotUpToDate();
                ...
                else if( _Key==TW_KEY_LEFT )
                {
                    if( !m_HierTags[m_HighlightedLine].m_Var->IsGroup() ) 
                    {
                        CTwVarAtom *Atom_KeyLeft = static_cast<CTwVarAtom *>(m_HierTags[m_HighlightedLine].m_Var);
                        bool Accept = !Atom_KeyLeft->m_NoSlider || Atom_KeyLeft->m_Type==TW_TYPE_BUTTON
                                      || Atom_KeyLeft->m_Type==TW_TYPE_BOOL8 || Atom_KeyLeft->m_Type==TW_TYPE_BOOL16 || Atom_KeyLeft->m_Type==TW_TYPE_BOOL32 || Atom_KeyLeft->m_Type==TW_TYPE_BOOLCPP
                                      || IsEnumType(Atom_KeyLeft->m_Type);
                        if( !Atom_KeyLeft->IsReadOnly() && Accept && !m_IsPopupList )
                        {
							Atom_KeyLeft->Increment(-1);
                            if( g_TwMgr==NULL ) // Mgr might have been destroyed by the client inside a callback call
                                return 1;
                            m_HighlightClickBtnAuto = g_TwMgr->m_Timer.GetTime();
                            NotUpToDate();
                ...
                else if( _Key==TW_KEY_RETURN )
                {
                    if( !m_HierTags[m_HighlightedLine].m_Var->IsGroup() ) 
                    {
                        CTwVarAtom *Atom_KeyReturn = static_cast<CTwVarAtom *>(m_HierTags[m_HighlightedLine].m_Var);
                        if( !Atom_KeyReturn->IsReadOnly() )
                        {
                            if(Atom_KeyReturn->m_Type==TW_TYPE_BUTTON || Atom_KeyReturn->m_Type==TW_TYPE_BOOLCPP
                                || Atom_KeyReturn->m_Type==TW_TYPE_BOOL8 || Atom_KeyReturn->m_Type==TW_TYPE_BOOL16 || Atom_KeyReturn->m_Type==TW_TYPE_BOOL32 )
                            {
                                bool isPopup =  m_IsPopupList;
								Atom_KeyReturn->Increment(+1);
                                if( g_TwMgr==NULL // Mgr might have been destroyed by the client inside a callback call
