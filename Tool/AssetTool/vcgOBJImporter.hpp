		...
		namespace io {
			...
			*/
			...
			public:
				...
				static int Open(OpenMeshType &mesh, const char *filename, int &loadmask, std::vector<ObjIndexedFace> &outIndexedFaces, CallBackPos *cb = 0)
				{
					Info oi;
					oi.mask = 0;
					oi.cb = cb;
					int ret = Open(mesh, filename, outIndexedFaces, oi);
					loadmask = oi.mask;
					return ret;
				}
				...
				*/
				static int Open(OpenMeshType &m, const char * filename, std::vector<ObjIndexedFace> &outIndexedFaces, Info &oi)
				{
					...
					stream.close();
					std::swap(indexedFaces, outIndexedFaces);
					return result;
				} // end of Open
