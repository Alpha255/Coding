...
void ApplicationCubemap::InitGeometriesResource()
{
	...
	g_Renderer->CreateVertexBuffer(s_Resource.GeometriesVBuf, sizeof(Vertex) * totalVertexCount,
	...
	g_Renderer->CreateShaderResourceView(s_Resource.StoneTex, "stone.dds");
	...
	const char *const pShaderName = "Cubemap.hlsl";
	...
	g_Renderer->CreatePixelShader(s_Resource.PixelShader, pShaderName, "PS_Main");
...
void ApplicationCubemap::InitCubemapResource()
{
	...
	g_Renderer->CreateTexture2D(cubemapTex, DXGI_FORMAT_R8G8B8A8_UNORM, s_CubemapSize, s_CubemapSize,
		D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE, 0U, 6U, 0U,
	...
	for (uint32_t i = 0U; i < 6U; ++i)
	{
		...
		g_Renderer->CreateRenderTargetView(s_Resource.DynamicCubemapRTVs[i], cubemapTex.Ptr(), &rtvDesc);
	}

	g_Renderer->CreateShaderResourceView(s_Resource.DynamicCubeMap, cubemapTex.Ptr(), DXGI_FORMAT_R8G8B8A8_UNORM, D3D11_SRV_DIMENSION_TEXTURECUBE);

	Ref<ID3D11Texture2D> depthTex;
	g_Renderer->CreateTexture2D(depthTex, DXGI_FORMAT_D32_FLOAT, s_CubemapSize, s_CubemapSize, D3D11_BIND_DEPTH_STENCIL);
	g_Renderer->CreateDepthStencilView(s_Resource.DynamicCubemapDSV, depthTex.Ptr(), DXGI_FORMAT_D32_FLOAT);
